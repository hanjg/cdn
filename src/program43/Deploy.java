...
//    	verticeInfo();
		...
		if (getMinCount > 0) {
			LogUtil.printLog("sumTime : " + getMinTime + " count : " + getMinCount + " mean : " + getMinTime / getMinCount);
		}
...
//			LogUtil.printLog("now initCount is " + initCount);
			...
			//层次聚类初始化
//			int need = numOfConsumers * 2 / 5;
//			int lessThan = numOfConsumers * 3 / 5;
...
//						randomDeleteSource(genes);
						int ms = -1;
						int mf = Infinity;
						for (int j = 0; j < numOfMinSource; j++) {
							int source = minSource[j];
	...
	private static void randomDeleteSource(char[] genes) {
		...
		int pos = random.nextInt(numOfCurSource);
		genes[pos] = '0';
	}
...
//			List<List<Integer>> situation = kMeans(validDistance, 1);
			//level获取源点组合
			int need = numOfConsumers * 2 / 5;
			int lessThan = numOfConsumers * 4 / 5;

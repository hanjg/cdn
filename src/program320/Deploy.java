...
//		int numOfPopulation = numOfVertice * 2;
		...
		final int stopLine = numOfPopulation / 2;
		int stopCount = 0;
		final int maxGenerationConst = 1000000;
...
//				nextGeneration[i + 1] = builder2.toString();
			...
			for (int i= 0; i < numOfPopulation; i++) {
				...
				if (temp < probability) {
					...
					int from = random.nextInt(sources.size());
					int to = random.nextInt(nextGeneration[0].length());
					if (from != to) {
						genes[from] = '0';
						genes[to] = '1';
					}
...
//			}
	...
	private static void printpopulation(String[] population, int count, boolean detail) {
		...
		logger.println("minCost :" + minCost + 
				"\nmaxFitness :" + history.get(minCode) + " number :" + maxCount);

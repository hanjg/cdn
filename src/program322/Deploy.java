...
//		final int enhanceLine = 0;
		final int enhanceLine = numOfPopulation * 4 / 5;
...
//		printOriginalGeneration(population);
		...
		for (generation = 0; generation < maxGenerationConst; generation++) {
			...
			LogUtil.printLog("start " + generation);
			...
			for (int i = 0; i < numOfPopulation; i++) {
				...
				LogUtil.printLog("cal fitness " + i + "fitness is " + fitness[i] + " total " + numOfPopulation);
			}
			
			LogUtil.printLog("fitnessed ");
...
//					}
			...
			LogUtil.printLog("muted");
			
			population = nextGeneration;
			
			LogUtil.printLog("finish " + generation);
...
//		final int stopConst = 10 * numOfVertice * numOfVertice;
	...
	private static void getResult() {
    	...
    	while (true) {
			if (!dfs(new boolean[numOfVertice + 2], numOfVertice, Infinity, new ArrayList<Integer>(defaultSize))) {
				break;
			}
		}
...
//			System.out.println();
			...
			for (GraphO.Edge e : edges.get(v1)) {
				if (e.end == v2) {
					logger.print(e);
					logger.println("   " + new GraphO.Edge(v1, v2, capacity, cost));
					logger.flush();
				}
			}

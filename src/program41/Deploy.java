...
{	
	...
	private static int maxTime = 7200000;
...
//		}
		...
		if (logger != null) {
	    	...
	    	if (count > 0) {
				...
				logger.println("sumTime : " + sumTime + " count : " + count + " mean : " + sumTime / count);
			}
		...
		if (count > 0) {
			LogUtil.printLog("\nsumTime : " + sumTime + " count : " + count + " mean : " + sumTime / count);
		}
...
//			}
//		}
		LogUtil.printLog("finish initPreparement  ");
...
//			LogUtil.printLog("now initCount is " + initCount);
			...
			situation = kMeans(validDistace, distance, numOfPopulation - 1);
...
//			int panish = minCount < enhanceLine ? (int) (11 - 10.0 / enhanceLine * minCount) : 1;
			if (minCount < enhanceLine) {
				...
				if (maxFitness > minFitness) {
					slope = (5.0 * maxFitness - minFitness / 2) / (maxFitness - minFitness);
					for (int i = 0; i < numOfPopulation; i++) {
...
//				}
				...
				if (temp < mutateProbability) {
					char[] genes = nextGeneration[i].toCharArray();
					if (population[i].equals(minCode)) {
						//随机移动源点至相邻节点
						int from = minSource[random.nextInt(numOfMinSource)];
...
//						genes[pos] = genes[pos] == '0' ? '1' : '0';
	...
	private static void printOriginalGeneration(String[] population) {
		...
		for (String unit : population) {
			...
			int cost = history.get(unit) == 0 ? -1 : standardCost - history.get(unit);
...
//			}
	...
	private static void sequentialCluster(int maxTime) {
		//消费节点(可以看做和汇点重合)和顶点的有效距离(i为消费节点id，j为顶点下标)
    	int[][] consumerDistace = new int[numOfVertice][numOfVertice];
		validDistace(consumerDistace);
...
//		final int stopConst = 10 * numOfVertice * numOfVertice;
		...
		for (clusterCount = 0; clusterCount < maxClusterConst; clusterCount++) {
			...
			//获取源点的组合
			List<List<Integer>> situation = kMeans(consumerDistace, distance, 1);
	    	...
	    	if (curCost != Infinity) {
	    		logger.println("clusterCount " + clusterCount + "cost " + curCost + " sourceSize " + numOfCurSource);
		    }
	    	
	    	LogUtil.printLog("clusterCount " + clusterCount + "cost " + curCost + " minCost " + minCost);
	    }
...
//		int count = 0;
		while (true) {
			...
			//前驱弧在arcs中的下标
			int[] preArc = new int[diNumOfVertice];
			//到superSource的距离
			int[] distance = new int[diNumOfVertice];
			...
			while (head != tail) {
				...
				for (int arcIndex = arcHeadIndex[start]; arcIndex != -1; arcIndex = arcs[arcIndex].next) {
					...
					//start,end能够到达起点，并且可以更新start->end的距离
					//pre[start] != -1
					if (remain[arcIndex] > 0 && distance[start] + cost < distance[end]) {
			...
			//无最短路径，已获得最小费用流
			if (distance[superSink] == Infinity) {
				break;
			}
...
//	}
	 ...
	 * @return
	 */
	private static List<List<Integer>> kMeans(int[][] validDistance, int[][] distace, int need) {
		//源点组合
    	List<List<Integer>> situation = new ArrayList<>(numOfConsumers);
    	
    	//开始聚类时存在num个源点
    	while (need-- > 0) {
			int num = random.nextInt(numOfConsumers * 3 / 5) + numOfConsumers * 2 / 5;
//    		int num = random.nextInt(numOfConsumers) + 1;
    		//源点的位置

...
{	
	...
	private static int maxTime = 85000;
...
//		}
    	...
    	Arrays.sort(capacityInfos, new Comparator<VertexInfo>() {
			...
			public int compare(VertexInfo o1, VertexInfo o2) {
				return o1.sumFlow > o2.sumFlow ? -1 : 1;
			}
		});
...
//		int numOfPopulation = numOfVertice * 2;
		...
		final int stopConst = numOfVertice * 100;
...
//					LogUtil.printLog("numofsources " + numOfCurSource + " cost " + curCost);
				...
				LogUtil.printLog("finish kmeans cluster");
			}
...
//			int addCount = minCount < 10 ? 10 - minCount : 0;
			...
			//惩罚非最优个体
			//增大适应度最大的个体的影响力
			int panish = numOfVertice < 200 ? 10 : 5;
			int maxFitness = 0;
			double slope= 0;
...
//			int panish = minCount < enhanceLine ? (int) (11 - 10.0 / enhanceLine * minCount) : 1;
			if (minCount < enhanceLine) {
				...
				if (maxFitness > minFitness) {
					int down = numOfVertice < 200 ? minFitness / 2 : 0;
					slope = (5.0 * maxFitness - down) / (maxFitness - minFitness);
					for (int i = 0; i < numOfPopulation; i++) {
						fitness[i] = (int) (slope * (fitness[i] - minFitness) + down);
					}
				}
			}
...
//			double randomConst = 1- numOfVertice / 1000.0;
			double randomConst = 0.5;
//			double mutateProbability = 0.2;
			
//			for (int i = 0; i < numOfPopulation; i++) {
//				calCost(population[i].toCharArray());
//			}

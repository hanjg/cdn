...
//		int numOfPopulation = numOfVertice * 2;
		...
		int[] fitness = null;
		int totalFitness = 0;
		final int stopConst = numOfVertice * numOfVertice / 10;
		int stopCount = 0;
		final int maxGenerationConst = 10000;
...
//		printPopulation(population, 0);
		...
		//遗传迭代
		int count = 0;
		for (count = 0; count < maxGenerationConst; count++) {
...
//			printPopulation(population, count);
			...
			totalFitness = sumFitness[numOfPopulation];
			
			//选择numOfPopulation-2个体
			for (int i = 0; i < numOfPopulation; i++) {
			...
			//开始和最后一个确定为适应度最大的个体
			nextGeneration[0] = fittestCode;
			nextGeneration[numOfPopulation - 1] = fittestCode;
			
			//交叉
			//除了开始和结尾保留，其余交叉
			for (int i = 1; i < numOfPopulation - 1; i+=2) {
...
//			if (convergenceCount == stopConst >> 1) {
...
//			}
		...
		LogUtil.printLog("-----------------------generationCount: " + count);

...
{	
	...
	private static int numOfVerticeWithMutiEdge;
	
	private static int numOfMultiEdge;
...
//		int numOfPopulation = numOfVertice * 2;
		...
		final int stopConst = numOfVerticeWithMutiEdge;
...
//		final int enhanceLine = 0;
    	...
    	int[][] distance = new int[numOfConsumers][numOfVerticeWithMutiEdge];
		...
		while (!inited) {
			...
			while (index < numOfPopulation / 2)  {
				situation = kMeans(distance);
...
//			}
			...
			double crossProbability = minCount < enhanceLine ? 0.8 : 1;
...
//			}
	...
	private static void sequentialCluster(int maxTime) {
		//消费节点(可以看做和汇点重合)和顶点的有效距离(i为消费节点id，j为顶点下标)
    	int[][] distance = new int[numOfVerticeWithMutiEdge][numOfVerticeWithMutiEdge];
		shortestDistance(distance);
...
//		final int stopConst = 10 * numOfVertice * numOfVertice;
	...
	private static void getResult() {
    	...
    	for (int source : minSource) {
			sourceEdges.add(new GraphO.Edge(numOfVerticeWithMutiEdge, source, Infinity, 0));
			graphO.numOfEdges++;
		}
    	edges.add(sourceEdges);
    	//虚拟汇点的邻接表
    	for (Consumer consumer : consumers) {
			edges.get(consumer.link).add(new GraphO.Edge(consumer.link, numOfVerticeWithMutiEdge + 1, consumer.demand, 0));
			graphO.numOfEdges++;
		}
    	edges.add(new ArrayList<GraphO.Edge>());
    	
		//有向图中的minFlow转化为无向图中的flow
		int[][] flow = new int[numOfVerticeWithMutiEdge][numOfVerticeWithMutiEdge];
		for (int i = numOfVerticeWithMutiEdge; i < 2 * numOfVerticeWithMutiEdge; i++) {//出节点
			for (int j = 0; j < numOfVerticeWithMutiEdge; j++) {//入节点
				flow[i - numOfVerticeWithMutiEdge][j] = minFlow[i][j];
			}
		}
    	int[][] temp = flow;
    	...
    	//复制原始网络的边
    	minFlow = new int[numOfVerticeWithMutiEdge + 2][numOfVerticeWithMutiEdge + 2];
    	for (int i = 0; i < numOfVerticeWithMutiEdge; i++) {
			for (int j =0; j < numOfVerticeWithMutiEdge; j++) {
    	...
    	//在流网络中增加超级源汇点对应的边
    	for (int source : minSource) {
			minFlow[numOfVerticeWithMutiEdge][source] = Infinity;
		}
    	for (Consumer consumer : consumers) {
			minFlow[consumer.link][numOfVerticeWithMutiEdge + 1] = consumer.demand;
		}
    	
    	result = new ArrayList<>(defaultSize);

    	while (true) {
			if (!dfs(new boolean[numOfVerticeWithMutiEdge + 2], numOfVerticeWithMutiEdge, Infinity, new ArrayList<Integer>(defaultSize))) {
				break;
			}
		}
     ...
     */
    private static boolean dfs(boolean[] visited, int cur, int flow, List<Integer> path) {
    	if (cur == numOfVerticeWithMutiEdge + 1) {
    		path.add(cur);
			StringBuilder builder = new StringBuilder();
			for (int i = 1; i < path.size() - 1; i++) {
				if (path.get(i) >= numOfVertice) {
					continue;
				}
    	...
    	for (GraphO.Edge edge : graphO.edges.get(cur)) {
			...
			if (!visited[next] && minFlow[cur][next] > 0) {
				if (dfs(visited, next, Math.min(flow, minFlow[cur][next]), path)) {
					return true;
				}
			}
		}
...
//			}
		...
		boolean enough = true;
		for (Consumer consumer : consumers) {
			int sink = consumer.link + numOfVerticeWithMutiEdge;
...
//			}
		...
		if (enough) {
			...
			for (int i = numOfVerticeWithMutiEdge; i < superSource; i++) {
				for (int j = 0; j < i - numOfVerticeWithMutiEdge; j++) {
					sumCost += (flowDi[i][j] + flowDi[j + numOfVerticeWithMutiEdge][i - numOfVerticeWithMutiEdge]) 
							* cost[i][j];
				}
			}
	...
	private static List<List<Integer>> kMeans(int[][] distance) {
    	...
    	for (int num = 1; num < numOfConsumers; num++) {
			...
			while (newSources.size() < num) {
				int temp = random.nextInt(numOfVerticeWithMutiEdge);
...
//			}
			...
			while (move) {
				...
				for (int source : map.keySet()) {
					...
					int newCenter = -1;
					int minSum = Infinity;
					for (int vertex = 0; vertex < numOfVerticeWithMutiEdge; vertex++) {
						int sum = 0;
...
//				distance[id][i] = (paths[i].distance << 16) / consumer.demand;
	...
	private static Path[] dijstra(List<List<GraphO.Edge>> edges, int v0) {
		Path[] paths = new Path[numOfVerticeWithMutiEdge];
		for (int vertex = 0; vertex < numOfVerticeWithMutiEdge; vertex++) {
			if (vertex == v0) {
				paths[vertex] = new Path(vertex, -2, 0);
			} else {
...
//				}
		...
		//存储没有确定最短路径的节点
		PriorityQueue<Path> heap = new PriorityQueue<>(numOfVerticeWithMutiEdge, new Comparator<Path>() {
...
//			System.out.println();
    	...
    	Deploy.numOfVerticeWithMutiEdge = numOfVertice + numOfMultiEdge;
		Deploy.diNumOfVertice = numOfVerticeWithMutiEdge * 2 + 2;
    	Deploy.numOfConsumers = numOfConsumers;
    	Deploy.superSource = 2 * numOfVerticeWithMutiEdge;
		...
		Deploy.cost = new int[2 * numOfVerticeWithMutiEdge][2 * numOfVerticeWithMutiEdge];
		
		for (List<GraphO.Edge> links : graphO.edges) {
			for (GraphO.Edge edge : links) {
				cost[edge.start + numOfVerticeWithMutiEdge][edge.end] = edge.cost;
			}
		}
		
		//初始化map，记录源点组合编码和对应的最小费用
		//初始化默认源点组合的编码
		history = new HashMap<>(numOfVerticeWithMutiEdge * numOfVerticeWithMutiEdge);
		defaultCode = new StringBuilder(numOfVerticeWithMutiEdge);
		for (int i = 0; i < numOfVerticeWithMutiEdge; i++) {
	...
	private static class GraphDi {
		...
		public GraphDi(List<Integer> sources) {
			if (arcsDi == null) {
				...
				//增加入节点到出节点的弧
				for (int vertex = 0; vertex < numOfVerticeWithMutiEdge; vertex++) {
					arcsDi.get(vertex).add(new Arc(vertex, vertex + numOfVerticeWithMutiEdge, Infinity, 0));
				}
				
				//无向边拆为两条有向弧
				for (int start = 0; start < numOfVerticeWithMutiEdge; start++) {
					for (GraphO.Edge edge : graphO.edges.get(start)) {
						int next = edge.end;
						//初始网络中一条边存储为起点和终点相反的两条弧
						if (start < next) {
							int in1 = start;
							int out1 = start + numOfVerticeWithMutiEdge;
							int in2 = next;
							int out2 = next + numOfVerticeWithMutiEdge;
				...
				for (Consumer consumer : consumers) {
					//出节点下标为原始节点下标+原始节点数
					int outSink = consumer.link + numOfVerticeWithMutiEdge;

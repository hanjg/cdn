...
//		LogUtil.printLog("minCost after heredity: " + minCost);
		
//    	List<Integer> curSources = new ArrayList<>(Arrays.asList(new Integer[]{7, 10, 12, 14, 16, 17, 19, 26, 27, 29, 31, 32, 35, 40, 43, 46, 50, 58, 59, 62, 69, 73, 75, 79, 82, 84, 86, 87, 88, 89, 96, 101, 107, 108, 110, 111, 113, 116, 119, 122, 124, 127, 129, 131, 134, 135, 137, 140, 146, 147, 152, 155, 158, 159, 161, 164, 166, 167, 168, 174, 175, 178, 179, 184, 185, 187, 189, 190, 194, 195, 196, 200, 203, 205, 208, 218, 223, 227, 228, 230, 233, 234, 237, 238, 241, 243, 245, 248, 252, 254, 259, 263, 266, 268, 269, 270, 277, 278, 281, 282, 286, 287, 298, 301, 308, 313, 318, 321, 323, 327, 328, 334, 335, 336, 338, 342, 346, 349, 359, 363, 366, 370, 375, 377, 381, 383, 384, 385, 387, 395, 396, 397, 402, 408, 413, 415, 416, 423, 426, 427, 432, 433, 437, 438, 441, 448, 457, 459, 463, 464, 469, 473, 475, 480, 482, 489, 495, 500, 507, 515, 520, 524, 525, 528, 529, 537, 542, 548, 551, 553, 556, 563, 565, 574, 576, 584, 589, 592, 599, 600, 624, 625, 626, 629, 632, 634, 636, 638, 640, 641, 642, 643, 644, 646, 651, 652, 653, 654, 660, 661, 663, 664, 668, 670, 671, 672, 675, 676, 677, 679, 684, 685, 687, 692, 695, 698, 710, 711, 714, 717, 718, 719, 721, 722, 724, 728, 730, 731, 735, 738, 739, 741, 742, 745, 751, 753, 755, 756, 757, 759, 760, 761, 764, 765, 767, 769, 770, 771, 773, 774, 775, 777, 778, 780, 784, 791, 793, 795, 796, 797, 799}));
//    	getMinFlow(curSources);
...
//		getMinFlow(extreme);
		...
		System.out.println("sumTime : " + sumTime + " count : " + count + " mean : " + sumTime / count);
...
//    	check();
	...
	private static void coolDown(int maxTime) {
		...
		int need = 100;
		List<List<Integer>> sources = kMeans(distance, need);
		
		int index = -1;
		while (++index < need) {
			double temperature = 100;
			double coolFactor = 0.99;

			List<Integer> originSources = sources.get(index);
			
			while (temperature > 0.1 && timer.getTimeDelay() < maxTime / need * (index + 1) ) {
				StringBuilder builder = new StringBuilder(defaultCode);
				for (int source : originSources) {
				...
				if (history.containsKey(curCode)) {
					fitOrigin = history.get(curCode);
				} else {
					getMinFlow(originSources);
					fitOrigin = curCost > standardCost ? 0 : (standardCost - curCost);
					history.put(curCode, fitOrigin);
				}
				
				
				LogUtil.printLog("now cool " + index + 
						" cost " + curCost + " standard " + standardCost + " fitorigin " + fitOrigin);
				...
				//搜索邻域，包括移动源点，增加和减少源点
				List<Integer> nextSources = null;
				
				LogUtil.printLog("start nearby index " + index);
				
				//移动源点
				int from = originSources.get(random.nextInt(originSources.size()));
				List<GraphO.Edge> links = graphO.edges.get(from);
				int to = from;
				while (originSources.contains(to)) {
					to = links.get(random.nextInt(links.size())).end;
				}
				
				List<Integer> moveSources = new ArrayList<>(originSources.size());
				for (int source : originSources) {
					if (source != from) {
				...
				moveSources.add(to);
				builder = new StringBuilder(defaultCode);
				for (int source : moveSources) {
					builder.setCharAt(source, '1');
				}
				curCode = builder.toString();
				...
				int fitMove = 0;
				if (history.containsKey(curCode)) {
					fitMove = history.get(curCode);
				} else {
					getMinFlow(moveSources);
					fitMove = curCost > standardCost ? 0 : (standardCost - curCost);
					history.put(curCode, fitMove);
				}
				
				double leapMove = 0;
				if (fitMove > 0) {
					leapMove = fitMove > fitOrigin ? 1 : Math.exp((fitMove - fitOrigin) / temperature);
					if (Math.random() < leapMove) {
						nextSources = moveSources;
					}
				}
				
				LogUtil.printLog("finish move index " + index + " fitness " + fitMove + " leap " + leapMove);
				
				//增加源点
				int add= from;
				while (originSources.contains(add)) {
					add = random.nextInt(numOfVertice);
				}
				List<Integer> addSources = new ArrayList<>(originSources);
				addSources.add(add);
				builder = new StringBuilder(defaultCode);
				for (int source : moveSources) {
					builder.setCharAt(source, '1');
				...
				int fitAdd = 0;
				if (history.containsKey(curCode)) {
					fitAdd = history.get(curCode);
				} else {
					getMinFlow(addSources);
					fitAdd = curCost > standardCost ? 0 : (standardCost - curCost);
					history.put(curCode, fitAdd);
				}
				
				double leapAdd = 0;
				if (fitAdd > 0) {
					leapAdd = fitAdd > fitOrigin ? 1 : Math.exp((fitAdd - fitOrigin) / temperature);
					if (Math.random() < leapAdd) {
						nextSources = moveSources;
					}
				}
				
				LogUtil.printLog("finish add index " + index + " fitness " + fitAdd + " leapAdd " + leapAdd);
				
				//减少源点
				int remove = from;
				while (originSources.contains(remove)) {
					remove = random.nextInt(numOfVertice);
				}
				List<Integer> removeSources = new ArrayList<>(originSources);
				removeSources.remove(new Integer(remove));
				builder = new StringBuilder(defaultCode);
				for (int source : removeSources) {
					builder.setCharAt(source, '1');
				}
				curCode = builder.toString();
				...
				int fitRemove = 0;
				if (history.containsKey(curCode)) {
					fitRemove = history.get(curCode);
				} else {
					getMinFlow(removeSources);
					fitRemove = curCost > standardCost ? 0 : (standardCost - curCost);
					history.put(curCode, fitRemove);
				}
				
				double leapRemove = 0;
				if (fitRemove > 0) {
					leapRemove = fitRemove > fitOrigin ? 1 : Math.exp((fitRemove - fitOrigin) / temperature);
					if (Math.random() < leapRemove) {
						nextSources = removeSources;
					}
				}
				
				LogUtil.printLog("finish remove index " + index + " fitness " + fitRemove + " leap " + leapRemove);
				
//				LogUtil.printLog("remove " + fitMove + " " + leapMove + 
//						" add " + fitAdd + " " + leapAdd + 
//						" remove " + fitRemove + " " + leapRemove);
//				
				originSources = nextSources;
	 ...
	 * @param curSources
	 */
	private static void chooseNearly(List<Integer> curSources) {
		StringBuilder builder = new StringBuilder(defaultCode);
    	for (int source : curSources) {
			builder.setCharAt(source, '1');
		}
    	minCode = builder.toString();
    	curCode = new String(builder);
    	getMinFlow(curSources);
		...
		int count = 0;
		for (int from : curSources) {
			for (GraphO.Edge edge : graphO.edges.get(from)) {
				int to = edge.end;
				if (curSources.contains(to)) {
					continue;
				}

				LogUtil.printLog("now : " + count++ + "from " + from + " to " + to);
				List<Integer> s = new ArrayList<>(curSources);
				s.remove(new Integer(from));
				s.add(to);
				builder = new StringBuilder(defaultCode);
				for (int source : s) {
					builder.setCharAt(source, '1');
				}
				String curCode = builder.toString();
				
				int fit2 = 0;
				if (history.containsKey(curCode)) {
					fit2 = history.get(curCode);
				} else {
					getMinFlow(s);
					fit2 = curCost > standardCost ? 0 : (standardCost - curCost);
					history.put(curCode, fit2);
				}
				logger.println(s + " cost: " + curCost + " fit: " + fit2);
				logger.flush();
...
//		final int enhanceLine = 0;
		final int enhanceLine = numOfPopulation * 3 / 10;
		...
		LogUtil.printLog("finish shortestDistance :");
...
//			LogUtil.printLog("now initCount is " + initCount);
			...
			situation = kMeans(distance, numOfPopulation - 1);
			for (List<Integer> source : situation) {
				if (index < numOfPopulation - 1) {
					StringBuilder builder = new StringBuilder(defaultCode);
					for (int gene : source) {
...
//				
//				sources = new ArrayList<>(sources.size());
//				for (int gene = 0; gene < genes.length; gene++) {
//					if (genes[gene] == '1') {
//						sources.add(gene);
//					}
//				}
//				LogUtil.printLog("init " + index);
//			}
...
//			}
			...
			//搜索邻域
//			for (int i = 0; i < numOfPopulation; i++) {
//				char[] genes = population[i].toCharArray();
//				List<Integer> midState = new ArrayList<>(defaultSize);
//				for (int gene = 0; gene < genes.length; gene++) {
//					if (genes[gene] == '1') {
//						midState.add(gene);
//					}
//				}
//				int fit1 = history.get(population[i]);
//				for (int from : midState) {
//					for (GraphO.Edge edge : graphO.edges.get(from)) {
//						int to = edge.end;
//						//可能会覆盖服务器
//						curCode = genes.toString();
//						int fit2 = 0;
//						if (history.containsKey(curCode)) {
//							fit2 = history.get(curCode);
//						} else {
//							List<Integer> sources = new ArrayList<>(defaultSize);
//							boolean containTo = false;
...
//				}
				...
				//之前错误，交换时未用temp。。
//				int cut1 = random.nextInt(nextGeneration[0].length() + 1);
...
//			double mutateProbability = minCount < enhanceLine ? 0.1 : 0.2;
			double mutateProbability = (0.2 / numOfPopulation) * minCount + 0.15;
...
//				}
	...
	private static void printOriginalGeneration(String[] population) {
		...
		for (String unit : population) {
			...
			if (history.get(unit) != 0) {
				validCount++;
			}
			logger.println("fitness : " + history.get(unit) + " size: " + sources.size() + " " + sources);
		}
		logger.println("valid unit amount:" + validCount);
		logger.println("-------------------------------");
		logger.flush();
	}
...
//		final int stopConst = 10 * numOfVertice * numOfVertice;
		...
		for (clusterCount = 0; clusterCount < maxClusterConst; clusterCount++) {
			...
			//获取源点的组合
			List<List<Integer>> situation = kMeans(distance, 1);
	    	...
	    	LogUtil.printLog("clusterCount " + clusterCount + " finish minCost " + minCost);
	    }
...
//				path.pre = -1;
//			}
//			paths[superSource].distance = 0;
//			paths[superSource].pre = -2;
			
			//节点是否在队列中
			boolean[] inQueue = new boolean[diNumOfVertice];
			int[] queue = new int[numOfVertice * 100];
			int head = 0;
			int tail = 0;
...
//			}
		...
		if (enough) {
			...
			//放弃没有输出的服务器???
//			sources = new ArrayList<>(originSources.size());
//			for (int source : originSources) {
//				if (flowDi[superSource][source] > 0) {
//					sources.add(source);
//				}
//			}
			
			sumCost += serverCost * sources.size();
			
//			if (originSources.size() != sources.size()) {
//				StringBuilder builder = new StringBuilder(defaultCode);
//				for (int source : sources) {
//					builder.setCharAt(source, '1');
//				}
...
//		LogUtil.printLog("prepare for next");
		...
		sumTime += System.currentTimeMillis() - time1;
		count++;
...
//		fitnessRatio = serverCost * numOfConsumers / 10000;
